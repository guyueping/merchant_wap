...
const MnLayout = (IProps: IPs) => {
    const { showTab = false, tabPath = '', children, title, gotoType = 2, hideStatusBar = false, showNav = true, path, statusBarStyle, restStyle, backTitle, } = IProps
    ...
    return (
        <View className='mnlayout_box' style={{ width: '100vw', height: '100vh', ...restStyle }}>
            ...
            { showNav ? <MnNav gotoType={gotoType} title={title} backTitle={backTitle} path={path} ></MnNav> : null
            }
